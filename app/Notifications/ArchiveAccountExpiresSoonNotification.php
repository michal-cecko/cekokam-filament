...
class ArchiveAccountExpiresSoonNotification extends Notification
{
    use Queueable;
    ...
    public function toMail(): MailMessage
    {
        return (new MailMessage)
            ->subject("Účet {$this->account->login} čoskoro expiruje")
            ->greeting('Expirácia účtu')
            ->line("Váš účet {$this->account->login} (s ID #{$this->account->id}) expiruje o {$this->account?->expiry_days} dní, dňa {$this->account->expires_at->format('j.n.Y')}.")
